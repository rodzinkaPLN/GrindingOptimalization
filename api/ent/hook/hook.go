...
// The UserinputFunc type is an adapter to allow the use of ordinary
// function as Userinput mutator.
type UserinputFunc func(context.Context, *ent.UserinputMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserinputFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserinputMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserinputMutation", m)
}
